...
const sealContentVariants: Variants = {
...
function fusingCopyForRarity(label: string) {
  if (label === "Mythic" || label === "Legendary") return "Rewriting the Archive...";
...
type ArtifactFilter = "all" | "common" | "uncommon" | "rare" | "epic" | "mythic";

function FusionCoreResultCard({ result }: { result: any }) {
  ...
  return (
    <div className={`fusion-core-result ${rarityClass}`} style={{ ["--fusion-reveal-ms" as any]: `${timing.revealMs}ms` }}>
      ...
      <div className="fusion-core-result__seal">
...
function SelectedSealBody({ item }: { item: Item }) {
  return (
    <div className="selected-seal-body">
      <div className="selected-seal-body__icon">
        {item.icon}
      </div>
      <div style={{ minWidth: 0 }}>
        <div className="selected-seal-body__name">{item.name}</div>
        <div className="selected-seal-body__rarity">
...
function SealSlot({
  ...
  return (
    <button
      className={`fusion-slot seal-slot is-${side} ${item ? "is-bound" : ""} ${active ? "is-active" : ""} ${dimmed ? "is-dimmed" : ""} ${shaking ? "is-error-shake" : ""}`}
      type="button"
      aria-pressed={active}
      disabled={disabled}
      onClick={onSelect}
    >
      <div className="seal-slot__header">
        <div className="seal-slot__label">{label}</div>
        {item && (
          <span
            className="seal-slot__clear"
            role="button"
      ...
      {item ? (
        <motion.div className="seal-slot__content" variants={sealContentVariants} animate={motionState}>
          <SelectedSealBody item={item} />
        </motion.div>
      ) : (
        <div className="seal-slot__empty">Choose a formula from the archive.</div>
      )}
    </button>
  );
}
...
function DiscoveryHintPanel({
  ...
  return (
    <div className="discovery-hint-panel">
      ...
      <div className="discovery-hint-panel__eyebrow">New paths unlocked</div>
      ...
      <div className="fusion-core-stage__shell">
        ...
        <div className="fusion-core-stage__core">
          ...
          {showResult && (
            ...
            >
              <FusionCoreResultCard result={result} />
            </motion.div>
          )}
        </div>
      </div>
...
function ArtifactCard({
  ...
  const rarity = rarityLabel(item.rarity_weight);
  const tone = rarityTone(rarity);
  const scarcity = artifactScarcity(item);

  return (
    <button
      className={`artifact-card ${selected ? "is-selected" : ""} ${disabled ? "is-disabled" : ""}`}
      key={item.id}
      type="button"
    ...
    >
      {selectedSlot && (
        <div className="artifact-card__selected-label">
          Selected as {selectedSlot === "left" ? "First Path" : "Second Path"}
        </div>
      )}
      <div className="artifact-card__topline">
        <div className="artifact-card__icon">{item.icon}</div>
        <RarityBadge item={item} />
      </div>
      <div className="artifact-card__name">{item.name}</div>
      <div className="artifact-card__subtitle">T{item.tier} - {craftRarityLabel(rarity)}</div>
      <div className="artifact-card__details">
        <span>Gen {artifactGeneration(item)}</span>
        <span>{artifactAffinity(item)}</span>
      </div>
      {scarcity && <div className="artifact-card__scarcity">{scarcity}</div>}
    </button>
  );
}

function ArtifactLedger({ items, left, right, inactive, onChoose }: { items: Item[]; left: number | null; right: number | null; inactive: boolean; onChoose: (itemId: number) => void }) {
  if (items.length === 0) {
    return <div className="artifact-ledger-empty">No discoveries are loaded yet.</div>;
  }

  return (
    <div className={`artifact-ledger item-ledger ${inactive ? "is-inactive" : ""}`}>
      {items.map((item) => (
        <ArtifactCard
          key={item.id}
          item={item}
...
function ProgressStatsRail({ items }: { items: Item[] }) {
...
function MobileCollectionDrawer({
  items,
  left,
  right,
  inactive,
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      const rarity = rarityLabel(item.rarity_weight).toLowerCase() as ArtifactFilter;
        ...
        {showFull && (
          <div className="mobile-collection-drawer__full">
            ...
            <div className="mobile-collection-drawer__filters" role="tablist" aria-label="Discovery filters">
              {(["all", "common", "uncommon", "rare", "epic", "mythic"] as ArtifactFilter[]).map((option) => (
                <button
                  key={option}
                  type="button"
    ...
    >
      {anticipating && selectedLeft && selectedRight && (
        <div className="fusion-orb-layer" aria-hidden="true">
          <div className="fusion-ghost-orb is-left">{selectedLeft.icon}</div>
          <div className="fusion-ghost-orb is-right">{selectedRight.icon}</div>
        </div>
      )}
...
export default function Home() {
  ...
  async function doCraft() {
    ...
    try {
      const ritualTiming = fusionTimingForRarity(strongerRarityLabel(selectedLeft, selectedRight));
      const craftPromise = demo ? Promise.resolve(demoCraft(left!, right!)) : api.craft(left!, right!);
      await sleep(ritualTiming.stageA);
      setRevealState("accelerating");
      await sleep(ritualTiming.stageB);
      setRevealState("locking");
      const [res] = await Promise.all([craftPromise, sleep(ritualTiming.stageC)]);
  ...
  function useResultAsSeal(slot: "left" | "right") {
    ...
    >
      ...
      <style>{`
        ...
        .fusion-chamber.is-fusing .fusion-orb {
          animation: ritualPulse 1.2s ease-in-out both;
        }
        ...
        .fusion-chamber.is-accelerating .seal-slot.is-bound,
        .fusion-chamber.is-locking .seal-slot.is-bound,
        .fusion-chamber.is-revealed .seal-slot.is-bound {
          opacity: 0.34;
        ...
        .seal-slot.is-error-shake {
          animation: sealSlotShake 250ms ease-in-out;
        }
        ...
        .seal-slot.is-active,
        .fusion-workspace.is-ready .seal-slot.is-bound,
        .fusion-workspace:not(.is-idle) .seal-slot.is-bound {
        ...
        .seal-slot.is-active .seal-slot__label {
          color: #d8bf82;
        }
        ...
        .seal-slot__clear:focus-visible,
        ...
        .artifact-ledger.is-inactive {
          pointer-events: none;
        }

        .artifact-ledger-empty {
          margin-top: 14px;
          padding: 18px;
          border-radius: 16px;
        ...
        .artifact-card.is-selected .artifact-card__icon {
          border-color: rgba(216,191,130,0.42);
        ...
        .artifact-card.is-disabled,
        .artifact-ledger.is-inactive .artifact-card {
          opacity: 0.54;
          pointer-events: none;
        ...
        .artifact-card__details span,
        .artifact-card__scarcity,
        .artifact-card__selected-label {
          border: 1px solid rgba(191,157,92,0.15);
          background: rgba(0,0,0,0.18);
        ...
        @media (max-width: 760px) {
          ...
          .mobile-collection-drawer .artifact-ledger {
            max-height: none;
            overflow: visible;
            padding-right: 0;
            grid-template-columns: repeat(2, minmax(0, 1fr));
      ...
      <nav style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
        <a href="/login" style={{ color: "#b8c4ff" }}>Login</a>
        <a href="/inventory" style={{ color: "#b8c4ff" }}>Inventory</a>
        <a href="/leaderboard" style={{ color: "#b8c4ff" }}>Leaderboard</a>
        {authed && (
          <button
            onClick={() => { demo ? exitDemo() : clearToken(); if (!demo) { setHasToken(false); location.href = "/login"; } }}
            style={{ marginLeft: "auto", background: "transparent", color: "#ffb4b4", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 10, padding: "6px 10px", cursor: "pointer" }}
          >
      ...
      <ArchiveChamber drawerState={drawerState} fusionActive={fusionModeActive}>
        ...
        <div className="archive-chamber__header">
          <div>
          <h2>Archive Chamber</h2>
          <p>Select two items, begin the fusion, and reveal what the archive resolves.</p>
        </div>
